...
class TokenAwareMemoryManager:
    ...
    def _load_from_db(self):
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT summary_json, history_buffer FROM conversation_memory WHERE user_id = %s AND session_number = %s",
...
class MemoryManager(TokenAwareMemoryManager):
    ...
    def add_interaction(self, prompt, response, full_response_for_history=None, original_prompt=None):
        """
