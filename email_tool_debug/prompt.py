...
1. **Call a function** → Set "function" to function name, provide "parameters"
2. **Complete the task** → Set "function" to null (this EXITS the loop)
...
- `after:YYYY-MM-DD` → Emails FROM that date onwards (inclusive)
- `before:YYYY-MM-DD` → Emails BEFORE that date (exclusive, not including that date)
- For emails on EXACTLY one day: Use BOTH `date_after` AND `date_before` (next day)

**Examples for {current_date}:**
- "emails from today" → {{"date_after": "{current_date}", "date_before": "{tomorrow_date}"}}
- "emails from yesterday" → {{"date_after": "{yesterday_date}", "date_before": "{current_date}"}}
- "emails in the last 7 days" → {{"date_after": "{seven_days_ago}"}}
...
❌ BAD: Guessing or using placeholders
✅ GOOD: Ask for the missing information

Examples:
- "Send email to John" → Ask: "What is John's email address?"
- "Reply to the email" → Ask: "Which email would you like me to reply to?"
- "Forward it to marketing" → Ask: "Which email should I forward, and what's the marketing team's email?"
